...
class NominatimService {
  Future<List<Map>> getAddressLatLng(String address) async {
    var url =
        "https://nominatim.openstreetmap.org/search?q=${address.replaceAll(RegExp(' '), '+')}&format=json&addressdetails=1&&polygon_geojson=1";
    print(url);
...
//    print('Response body: ${response.body}');
    ...
    for (Map ad in addresses) {
      share.add({
        ...
        'polygonCoordinates': ad['geojson']['coordinates'] == null ? "na" : ad['geojson']['coordinates']
      });
    }
    print("Share $share");
    return share;
